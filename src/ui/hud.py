...
from src.quickcraft.library import (
    ...
    update_hotkey as update_quickcraft_hotkey,
    load_global_hotkey,
    save_global_hotkey,
)
...
class BuffHUD:
    ...
    def __init__(
        ...
        self._library_nb = ttk.Notebook(
            self._tab_library_group_frame, style="TNotebook"
        )
        ...
        try:
            ttk.Label(
                ...
                text=t(
                    "desc.settings", "Configure ROI, focus policy, dock and language."
                ),
                style="Prompt.TLabel",
        ...
        try:
            ttk.Label(
                ...
                text=t(
                    "desc.overview",
                    "Start/stop scanning and positioning; view detected templates.",
                ),
                style="Prompt.TLabel",
            ).pack(anchor="w", padx=12, pady=(8, 4))
        except Exception:
        ...
        self._root_notebook.add(
            self._tab_overview_frame, text=t("tab.overview", "Overview")
        )
        self._root_notebook.add(
            self._tab_library_group_frame, text=t("tab.library_group", "Library")
        )
        self._root_notebook.add(
        ...
        try:
            ttk.Label(
                ...
                text=t("desc.library", "Maintain items: Buffs, Debuffs, Copy Areas."),
                style="Prompt.TLabel",
            ).pack(anchor="w", padx=12, pady=(8, 4))
        except Exception:
            pass
        try:
            ttk.Label(
                ...
                text=t(
                    "desc.tools",
                    "Runtime tools: Currency overlay, Quick Craft, Mega QoL.",
                ),
                style="Prompt.TLabel",
            ).pack(anchor="w", padx=12, pady=(8, 4))
        except Exception:
        ...
        self._library_nb.add(
            self._tab_copy_frame, text=t("tab.copy_area", "Copy Areas")
        )
        ...
        self._buffs_tab.get_tree_view().get_search_var().trace_add(
            "write", lambda *args: self._reload_library()
        )
    ...
    def _on_topmost_changed(self) -> None:
        ...
        try:
            self._root.attributes(
                "-topmost", bool(self._settings_tab.get_topmost_var().get())
            )
        except Exception:
            pass
    ...
    def _on_toggle_positioning(self) -> None:
        ...
        self._events.append(
            "POSITIONING_ON"
            if self._monitoring_tab.get_positioning_var().get()
    ...
    def _on_toggle_currency_positioning(self, enabled: bool) -> None:
        ...
        self._events.append(
            "CURRENCY_POSITIONING_ON" if enabled else "CURRENCY_POSITIONING_OFF"
        )
    ...
    def _on_add_entry(self, entry_type: str) -> None:
        ...
        entry = make_entry(
            ...
            description_en=res["description"].get(
                "en", res["description"].get(get_lang(), "")
            ),
    ...
    def _on_delete_entry(self, entry_type: str) -> None:
        ...
        confirm_key = (
            "library.confirm_delete_buff"
            if entry_type == "buff"
            else "library.confirm_delete_debuff"
        )
        try:
        ...
        if not delete_entry(entry_id, entry_type):
            try:
                messagebox.showerror(
                    title="Error",
                    message=t("error.delete_failed", "Unable to delete selected item"),
                )
            except Exception:
                pass
            return
    ...
    def _on_edit_currency(self) -> None:
        ...
        if not success:
            try:
                messagebox.showerror(
                    title="Error",
                    message=t("error.save_failed", "Unable to save changes"),
                )
            except Exception:
                pass
            return
    ...
    def _on_quickcraft_set_hotkey(self, _currency_id: str) -> None:
        ...
        self._quickcraft_tab.start_hotkey_capture(
            lambda token: self._apply_global_hotkey(token)
        )
    ...
    def _reload_library(self) -> None:
        ...
        if quick_query:
            ...
            for entry in currencies:
                haystack = (
                    f"{entry.get('name', '')} {entry.get('interface', '')}".lower()
                )
                if q in haystack:
    ...
    def _refresh_texts(self) -> None:
        ...
        try:
            ...
            self._library_nb.tab(
                self._tab_debuffs_frame, text=t("tab.debuffs", "Debuffs")
            )
            self._library_nb.tab(
                self._tab_copy_frame, text=t("tab.copy_area", "Copy Areas")
            )
            self._tools_nb.tab(
                self._tab_currency_frame, text=t("tab.currency", "Currency")
            )
            self._tools_nb.tab(
                self._tab_quickcraft_frame, text=t("tab.quickcraft", "Quick Craft")
            )
    ...
    def _on_edit_copy_area(self) -> None:
        ...
        update_copy_area_entry(
            area_id,
            {
                ...
                "width": res["width"],
                "height": res["height"],
                "transparency": res.get(
                    "transparency", current.get("transparency", 1.0)
                ),
                "topmost": res.get("topmost", True),
            },
        )
    ...
    def _on_delete_copy_area(self) -> None:
        ...
        try:
            confirm = messagebox.askyesno(
                title="Confirm",
                message=t(
                    "copy_area.confirm_delete", "Are you sure you want to delete it?"
                ),
            )
        except Exception:
            confirm = True
